...
class DayuUpload(object):
    ...
    def __init__(self, account, password, video_type, video_path, image_path, title, content, id, tags, is_origin):
        self.id = id
        self.tags = tags
    ...
    def go(self):
        ...
        if self.is_origin == 1:
            try:
                self.browser.find_element_by_xpath("//*[@class='article-write_video_box-original']/*[@class='w-btn w-btn_primary']").click()
                # / html / body / div[1] / div[4] / div / div[2] / div / div / div[1] / div[3] / div[1] / button
                time.sleep(2)
                self.browser.find_element_by_xpath("//*[@class='article-write-openOriginal_check']/*[@class='w-radio']").click()
                time.sleep(2)
                self.browser.find_element_by_xpath("//*[@class='article-write-openOriginal_footer']/*[@class='w-btn w-btn_primary']").click()
            except Exception:
                self.msg = '选择原创失败, 请重新提交'
                return
